    ...
    async with state.proxy() as data:
        ...
        for i in data_user:
            try:
                ...
                count_error+=1
                await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=f'*Данные о рассылки\nТекст: "{message.text}"\n✅Успешно: {count_accept}\n❌Ошибки: {count_error}*', parse_mode=types.ParseMode.MARKDOWN_V2)
        await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=f'*Данные о рассылки\nТекст: "{message.text}"\n✅Успешно: {count_accept}\n❌Ошибки: {count_error}\nРассылка завершенна🔔*', parse_mode=types.ParseMode.MARKDOWN_V2, reply_markup=ikb_close)
    await state.finish()
...
async def chennger_wellcome_text(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        try:
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=message.text, parse_mode=types.ParseMode.MARKDOWN_V2)
